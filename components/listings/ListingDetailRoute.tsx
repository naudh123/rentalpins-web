...
import { fetchSaleListingIntelligence, saleCompsMapSearchHref } from "@/lib/sale/listing-comps";
...
export type ListingDetailChannel = "rent" | "buy";
...
export default async function ListingDetailRoute({
  ...
  const mapHref = isSale
    ...
    : appPath(mapSearchUrl(listing.lat, listing.lng, 14, listingId));
  const similarMapHref = isSale
    ? appPath(
        saleCompsMapSearchHref(listing.lat, listing.lng, {
          zoom: 13,
          bhk: listing.attributes?.bhk,
        })
      )
    : appPath(mapSearchUrl(listing.lat, listing.lng, 13, undefined, listing.category));

  return (
    <article
      className={`mx-auto max-w-3xl px-4 py-6 pb-28 sm:py-8 sm:pb-8${isSale ? " sale-theme" : ""}`}
      data-transaction={isSale ? "sale" : undefined}
    >
      <script
          ...
          <p className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-[var(--muted)]">
            ...
            <ListingMapLink
              ...
              className={`text-xs font-semibold hover:underline ${
                isSale ? "text-[var(--sale-gold)]" : "text-[var(--brand-orange)]"
              }`}
            >
              View on {isSale ? "buy " : ""}map
            </ListingMapLink>
          </p>
        )}
        <div className="mt-4 flex items-start justify-between gap-3">
          <p className={`font-serif text-2xl ${isSale ? "text-[var(--sale-gold)]" : "text-[var(--brand-orange)]"}`}>
            {priceLabel}
          </p>
        ...
        >
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex shrink-0 flex-col items-end gap-1">
              ...
              >
                View similar on {isSale ? "buy " : ""}map →
              </ListingMapLink>
            </div>
          </div>
          <div className="grid gap-3">
