...
import {
  resolveBlogAuthorBio,
  resolveBlogAuthorName,
} from "@/lib/seo/blog-author";
...
export default function BlogAuthorBox({
  ...
  const resolvedName = resolveBlogAuthorName(authorName);
  const bio = resolveBlogAuthorBio(resolvedName, category, vertical);
  const moreHref = blogIndexHref(vertical);
  ...
  return (
    <aside className="mt-12 rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-6">
      ...
      <p className="mt-3 font-serif text-xl font-bold text-[#1E3A6E]">{resolvedName}</p>
      <p className="mt-2 text-sm leading-relaxed text-slate-600">{bio}</p>
      <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-slate-500">
        <span
          className={`rounded-full px-3 py-1 font-semibold ${verticalBadgeClass(vertical)}`}
        >
