...
export default function PostAuthGate({ listingId = null }: Props) {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      router.replace(appPath(postNext));
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-12">
      ...
      <div className="rp-card mt-8 space-y-4 p-6">
        ...
        <Link
          href={loginHref}
          className="block w-full rounded-full border border-[var(--border)] px-4 py-3 text-center text-sm font-medium text-[var(--brand-navy)] hover:bg-[var(--surface-hover)]"
        >
